#!/usr/bin/env python3
"""
Music Soundboard with Recording - Keyboard-controlled sampler for making techno/electronic music
"""
    ...
    def mixer_loop(self):
        ...
        while self.mixer_running:
            ...
            if self.is_recording:
                self.record_buffer.append(self.mix_buffer.copy())
                elapsed = (len(self.record_buffer) * 2048) / self.sample_rate
                self.recording_time.emit(f"{int(elapsed)}s")
...
class SoundboardApp(QMainWindow):
    """Main soundboard application with recording"""

    # Keyboard to grid mapping
    KEY_MAP = {
    ...
    def __init__(self):
        ...
        self.audio_engine.recording_time.connect(self.update_recording_time)
        self.buttons = {}
        self.sound_colors = {}
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("🎵 Music Soundboard with Recording")
        ...
        # BPM control
        right_layout.addWidget(QLabel("⏱️ BPM"))
        bpm_spin = QSpinBox()
    ...
    def update_recording_time(self, time_str):
        """Update recording time display"""
        if self.recording:
            self.record_time.setText(f"Recording: {time_str}")
            ...
            for row in range(4):
                for col in range(4):
                    if (row, col) not in self.audio_engine.sounds or len(self.audio_engine.sounds[(row, col)]) == 0:
                        ...
                        return
            QMessageBox.warning(self, "Full", "All buttons filled!")
